...
## CPU usage stats for multiple servers using `mpstat`

``` bash
mpstat | sed -E -n "N;N;N; s/^Linux.*\(([a-z]+)\).*\(([0-9]+) CPU\).*(CPU.*%idle).*all/host\t\3\n\1\t\2/; p;"

parallel --jobs 6 --keep-order ssh {} mpstat ::: bork thornton eddy lecun leunissen handelsman myers  2>/dev/null | sed -E -n "N;N;N; s/^Linux.*\(([a-z]+)\).*\(([0-9]+) CPU\).*(CPU.*%idle).*all/host\t\3\n\1\t\2/; p;" | awk 'NR == 1 {print $0}; $1 != "host" { print $0 | "sort -n -k3,3" }' | column -t -s $'\t'

```

## Perl one liners

Source: http://www.catonmat.net/blog/perl-one-liners-explained-part-six/
